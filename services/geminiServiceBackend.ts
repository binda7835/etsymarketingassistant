...
const API_URL = (import.meta as any).env?.VITE_API_URL || '/api/gemini';
...
export const findAudienceOnline = async (keywords: string): Promise<GroundedContent> => {
  const prompt = `
    ...
    `;
  try {
    const result = await callGeminiAPI(prompt, 'gemini-2.5-flash', true);
    return {
      text: result.text,
    ...
    return {
      text: "An error occurred while searching for audience communities. Please check the console for details.",
      sources: [],
    };
  }
};
...
export const generateContentCalendar = async (persona: string): Promise<string> => {
  ...
  `;
  try {
    const result = await callGeminiAPI(prompt, 'gemini-2.5-pro');
    return result.text;
  } catch (error) {
...
export const getEtsyMarketResearch = async (niche: string): Promise<GroundedContent> => {
  ...
  `;
  try {
    const result = await callGeminiAPI(prompt, 'gemini-2.5-pro', true);
    return {
      text: result.text,
      sources: result.sources || [],
    };
    ...
    return {
      text: "An error occurred while researching the Etsy market. Please check the console for details.",
      sources: [],
    };
  }
};
...
export const generateReviewResponse = async (review: string): Promise<string> => {
  ...
  `;
  try {
    const result = await callGeminiAPI(prompt, 'gemini-2.5-flash');
    return result.text;
  } catch (error) {
