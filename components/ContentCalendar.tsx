...
import { generateContentCalendar } from '../services/geminiServiceFree';
...
const ContentCalendar: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      setCalendar('Failed to create content calendar. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold text-white mb-2">Create a Content Calendar</h2>
      <p className="text-gray-400 mb-6">Provide a persona to outline a full 7-day content plan for Instagram and Pinterest. Get daily ideas, captions, and hashtags to save you hours of work.</p>
      ...
      <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-lg border border-gray-700">
         <div className="mb-4">
            ...
            <textarea
                ...
                placeholder="Paste the persona details here (from the 'Create Audience Personas' tool) to get a tailored content plan..."
            />
        </div>
        <div className="bg-blue-900/50 border border-blue-700 text-blue-300 text-xs rounded p-3 mb-4">
            <i className="fas fa-info-circle mr-2"></i>
            <strong>How to Use:</strong> After creating your plan, simply copy the text for each day and post it on the recommended platform. This keeps you in control and your accounts safe!
        </div>
        <button
          type="submit"
        ...
        >
          {isLoading ? 'Creating Calendar...' : <><i className="fas fa-calendar-alt"></i> Create Content Plan</>}
        </button>
      </form>
